// pc-receiver.js - Receptor local para PC con notificaciones en tiempo real
// Ejecutar con: node dev/pc-receiver.js
...
// Configuración
// Por defecto conectar al dominio público/puerto expuesto
...
// Mostrar notificación en Windows usando PowerShell
function showNotification(title, message) {
  try {
    ...
    // Fallback: intentar con msg.exe
    try {
      execSync(`msg * "Nuevo registro: ${message.substring(0, 100)}"`, {
        timeout: 3000,
        stdio: "ignore",
      });
    } catch (e2) {
      console.log("No se pudo mostrar notificación nativa");
    }
  }
}
...
async function saveRegistration(data) {
  try {
    ...
    // También guardar un archivo individual por registro
...
socket.on("connect", () => {
  ...
  showNotification(
    "SmokeAndGo - Conectado",
    "Receptor de registros activo. Esperando nuevos usuarios..."
  );
});
...
socket.on("new-registration", (data) => {
  ...
  console.log("");
  console.log(`🆕 NUEVO REGISTRO - ${timestamp}`);
  console.log(`   👤 Nombre: ${name}`);
  console.log(`   📧 Email: ${email}`);
  console.log(`   🌐 IP: ${ip}`);
  if (data.device) {
    console.log(`   📱 Dispositivo: ${data.device.modelName || "N/A"} ${data.device.brand || ""}`);
  }
  console.log("");
